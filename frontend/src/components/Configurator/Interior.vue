      ...
      <div class="tw-w-full tw-flex tw-justify-between tw-items-center">
        <div class="tw-w-1/4">
          <img
            ...
            :alt="interior.name"
            class="tw-object-contain tw-rounded-xl"
          />
        </div>
        <div class="tw-w-2/3">
          <div class="tw-text-2xl tw-mb-10">
            {{ t("Interior.price") }}
            {{ n(getBrutto(interior.price, taxes), "currency", currency) }}
          </div>
          <div class="tw-flex tw-w-1/2 tw-h-10">
            <button
              type="button"
              class="btn btn-primary tw-w-1/6 tw-rounded-l-lg tw-rounded-r-none"
              @click="removeFromConfiguration()"
            >
              -
            </button>
            <div
            ...
            <button
              type="button"
              class="btn btn-primary tw-w-1/6 tw-rounded-r-lg tw-rounded-l-none"
              @click="addToConfiguration()"
            >
              +
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    removeFromConfiguration() {
      this.$store.commit("removeFromConfiguration", {
        interior: this.interior,
        configId: this.configId,
        product: this.product,
      });
